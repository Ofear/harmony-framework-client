    ...
    try {
        ...
        if (errorCode) {
            ...
            if (errorData && errorData.component.toLocaleLowerCase() !== BaseComponentTypes.IGNORE) {
                Store.dispatch({payload: errorData, type: TypesNames.ERROR_HANDLER_INVOKE});
            }
        } else {
            Store.dispatch({payload: {component: BaseComponentTypes.ERROR_PAGE}, type: TypesNames.ERROR_HANDLER_INVOKE});
        }
    } catch (e) {
