    ...
    return XMLHttpRequest.prototype.open = function (method: any, url: string) {
        this.addEventListener('readystatechange', function () {
            ...
            if (this.readyState === this.OPENED) {
                if (matchItem && matchItem.length) store.dispatch({type: 'XHR_TASK_START', [ pendingTask ]: begin});
            } else if (this.readyState === this.DONE) {
                if (matchItem && matchItem.length) store.dispatch({type: 'XHR_TASK_END', [ pendingTask ]: end});
            }
        }, false);
